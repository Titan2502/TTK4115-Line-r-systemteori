 ...
 * C source code generated on : Fri Sep 28 15:00:57 2018
 *
...
void P2p1_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P2p1_M)) {
    ...
    if (P2p1_B.RateTransitionx > P2p1_P.DeadZonex_End) {
      rtb_Frontgain = P2p1_B.RateTransitionx - P2p1_P.DeadZonex_End;
    } else if (P2p1_B.RateTransitionx >= P2p1_P.DeadZonex_Start) {
      rtb_Frontgain = 0.0;
    } else {
      rtb_Frontgain = P2p1_B.RateTransitionx - P2p1_P.DeadZonex_Start;
    }
     ...
     *  Gain: '<S4>/Gain: x'
     */
    P2p1_B.Joystick_gain_x = P2p1_P.Gainx_Gain * rtb_Frontgain *
      P2p1_P.Joystick_gain_x;

    /* Gain: '<Root>/R2D' */
    P2p1_B.R2D = P2p1_P.R2D_Gain * P2p1_B.Joystick_gain_x;
    ...
    if (rtmIsMajorTimeStep(P2p1_M)) {
      {
        if (!(++P2p1_DW.ToFile_IWORK.Decimation % 1) &&
            (P2p1_DW.ToFile_IWORK.Count*2)+1 < 100000000 ) {
          FILE *fp = (FILE *) P2p1_DW.ToFile_PWORK.FilePtr;
          if (fp != (NULL)) {
            real_T u[2];
            P2p1_DW.ToFile_IWORK.Decimation = 0;
            u[0] = P2p1_M->Timing.t[1];
            u[1] = P2p1_B.Joystick_gain_x;
            if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
              rtmSetErrorStatus(P2p1_M,
                                "Error writing to MAT-file pitch_reference_p2p1.mat");
              return;
            }

            if (((++P2p1_DW.ToFile_IWORK.Count)*2)+1 >= 100000000) {
              (void)fprintf(stdout,
                            "*** The ToFile block will stop logging data before\n"
                            "    the simulation has ended, because it has reached\n"
                            "    the maximum number of elements (100000000)\n"
                            "    allowed in MAT-file pitch_reference_p2p1.mat.\n");
            }
          }
        }
      }
   ...
   */
  P2p1_B.Gain_d = (P2p1_P.TravelTransferFcn_C * P2p1_X.TravelTransferFcn_CSTATE
   ...
   */
  P2p1_B.Gain_b = (P2p1_P.PitchTransferFcn_C * P2p1_X.PitchTransferFcn_CSTATE +
  ...
  if (rtmIsMajorTimeStep(P2p1_M)) {
    ...
    /* Gain: '<S7>/Gain' */
    P2p1_B.Gain_e = P2p1_P.Gain_Gain_lv * P2p1_B.ElevationCounttorad;

    /* Sum: '<Root>/Sum1' incorporates:
     *  Constant: '<Root>/offset_e'
     */
    P2p1_B.Sum1 = P2p1_B.Gain_e + P2p1_P.offset_e_Value;
  }

  /* Gain: '<S8>/Gain' incorporates:
   *  TransferFcn: '<S3>/Elevation: Transfer Fcn'
   */
  P2p1_B.Gain_dg = (P2p1_P.ElevationTransferFcn_C *
                    P2p1_X.ElevationTransferFcn_CSTATE +
                    P2p1_P.ElevationTransferFcn_D * P2p1_B.ElevationCounttorad) *
    P2p1_P.Gain_Gain_n;

  /* Gain: '<Root>/Deg_2_rad' */
  P2p1_B.Deg_2_rad[0] = P2p1_P.Deg_2_rad_Gain * P2p1_B.Gain_p;
  P2p1_B.Deg_2_rad[1] = P2p1_P.Deg_2_rad_Gain * P2p1_B.Gain_d;
  P2p1_B.Deg_2_rad[2] = P2p1_P.Deg_2_rad_Gain * P2p1_B.Gain;
  P2p1_B.Deg_2_rad[3] = P2p1_P.Deg_2_rad_Gain * P2p1_B.Gain_b;
  P2p1_B.Deg_2_rad[4] = P2p1_P.Deg_2_rad_Gain * P2p1_B.Sum1;
  P2p1_B.Deg_2_rad[5] = P2p1_P.Deg_2_rad_Gain * P2p1_B.Gain_dg;
  if (rtmIsMajorTimeStep(P2p1_M)) {
    /* ToFile: '<Root>/To File1' */
    if (rtmIsMajorTimeStep(P2p1_M)) {
      {
        if (!(++P2p1_DW.ToFile1_IWORK.Decimation % 1) &&
            (P2p1_DW.ToFile1_IWORK.Count*7)+1 < 100000000 ) {
          FILE *fp = (FILE *) P2p1_DW.ToFile1_PWORK.FilePtr;
          if (fp != (NULL)) {
            real_T u[7];
            P2p1_DW.ToFile1_IWORK.Decimation = 0;
            u[0] = P2p1_M->Timing.t[1];
            u[1] = P2p1_B.Deg_2_rad[0];
            u[2] = P2p1_B.Deg_2_rad[1];
            u[3] = P2p1_B.Deg_2_rad[2];
            u[4] = P2p1_B.Deg_2_rad[3];
            u[5] = P2p1_B.Deg_2_rad[4];
            u[6] = P2p1_B.Deg_2_rad[5];
            if (fwrite(u, sizeof(real_T), 7, fp) != 7) {
              rtmSetErrorStatus(P2p1_M,
                                "Error writing to MAT-file state_output_p2p1.mat");
              return;
            }

            if (((++P2p1_DW.ToFile1_IWORK.Count)*7)+1 >= 100000000) {
              (void)fprintf(stdout,
                            "*** The ToFile block will stop logging data before\n"
                            "    the simulation has ended, because it has reached\n"
                            "    the maximum number of elements (100000000)\n"
                            "    allowed in MAT-file state_output_p2p1.mat.\n");
            }
          }
        }
      }
   ...
   *  Sum: '<S5>/Sum'
   */
  rtb_Frontgain = (P2p1_B.Joystick_gain_x - P2p1_B.Deg_2_rad[2]) * P2p1_P.K_pp -
    P2p1_P.K_pd * P2p1_B.Deg_2_rad[3];
  ...
  if (rtmIsMajorTimeStep(P2p1_M)) {
    /* Constant: '<Root>/e_ref' */
    P2p1_B.e_ref = P2p1_P.e_ref_Value;
  }

  /* Sum: '<S2>/Sum' */
  rtb_Sum_p = P2p1_B.e_ref - P2p1_B.Deg_2_rad[4];
  if (rtmIsMajorTimeStep(P2p1_M)) {
    /* Constant: '<Root>/Vs_const' */
    P2p1_B.Vs_const = P2p1_P.V_s;
  }
  ...
  rtb_Backgain = ((P2p1_P.K_ep_Gain * rtb_Sum_p + rtb_Backgain) -
                  P2p1_P.K_ed_Gain * P2p1_B.Deg_2_rad[5]) + P2p1_B.Vs_const;
  ...
  if (rtmIsMajorTimeStep(P2p1_M)) {
    ...
    if (P2p1_B.RateTransitiony > P2p1_P.DeadZoney_End) {
      rtb_Frontgain = P2p1_B.RateTransitiony - P2p1_P.DeadZoney_End;
    } else if (P2p1_B.RateTransitiony >= P2p1_P.DeadZoney_Start) {
      rtb_Frontgain = 0.0;
    } else {
      rtb_Frontgain = P2p1_B.RateTransitiony - P2p1_P.DeadZoney_Start;
    }
     ...
     *  Gain: '<S4>/Gain: y'
     */
    P2p1_B.Joystick_gain_y = P2p1_P.Gainy_Gain * rtb_Frontgain *
...
void P2p1_initialize(void)
{
  ...
  {
    char fileName[509] = "pitch_reference_p2p1.mat";
    FILE *fp = (NULL);
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(P2p1_M,
                        "Error creating .mat file pitch_reference_p2p1.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,2,0,"pitch_ref")) {
      rtmSetErrorStatus(P2p1_M,
                        "Error writing mat file header to file pitch_reference_p2p1.mat");
      return;
    }
  ...
  {
    char fileName[509] = "state_output_p2p1.mat";
    FILE *fp = (NULL);
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(P2p1_M, "Error creating .mat file state_output_p2p1.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,7,0,"states")) {
      rtmSetErrorStatus(P2p1_M,
                        "Error writing mat file header to file state_output_p2p1.mat");
      return;
    }
...
void P2p1_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      ...
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(P2p1_M,
                          "Error closing MAT-file pitch_reference_p2p1.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(P2p1_M,
                          "Error reopening MAT-file pitch_reference_p2p1.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 2, P2p1_DW.ToFile_IWORK.Count, "pitch_ref"))
      {
        rtmSetErrorStatus(P2p1_M,
                          "Error writing header for pitch_ref to MAT-file pitch_reference_p2p1.mat");
      }
  ...
  /* Terminate for ToFile: '<Root>/To File1' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "state_output_p2p1.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(P2p1_M, "Error closing MAT-file state_output_p2p1.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(P2p1_M,
                          "Error reopening MAT-file state_output_p2p1.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 7, P2p1_DW.ToFile1_IWORK.Count, "states"))
      {
        rtmSetErrorStatus(P2p1_M,
                          "Error writing header for states to MAT-file state_output_p2p1.mat");
      }
...
RT_MODEL_P2p1_T *P2p1(void)
{
  ...
  /* External mode info */
  P2p1_M->Sizes.checksums[0] = (609361253U);
  P2p1_M->Sizes.checksums[1] = (2172279470U);
  P2p1_M->Sizes.checksums[2] = (739157149U);
  P2p1_M->Sizes.checksums[3] = (624172865U);
  ...
  P2p1_M->Sizes.numBlocks = (58);      /* Number of blocks */
  P2p1_M->Sizes.numBlockIO = (23);     /* Number of block outputs */
  P2p1_M->Sizes.numBlockPrms = (154);  /* Sum of parameter "widths" */
  return P2p1_M;
}
