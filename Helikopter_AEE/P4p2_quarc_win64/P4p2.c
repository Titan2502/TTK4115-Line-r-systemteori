...
void P4p2_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P4p2_M)) {
    ...
    P4p2_B.Vs_const = P4p2_P.V_s;
  }

  /* Sum: '<Root>/Sum' */
  rtb_Backgain = P4p2_B.Vs_const + rtb_TmpSignalConversionAtGain1I[0];
  if (rtmIsMajorTimeStep(P4p2_M)) {
  }
  ...
  /* Sum: '<S12>/Sum2' */
  P4p2_B.Sum2_l = P4p2_B.Integrator[0] - P4p2_B.Joystick_gain_x;

  /* Sum: '<S12>/Sum' */
  P4p2_B.Sum = P4p2_B.Integrator[3] - P4p2_B.Joystick_gain_y;

  /* SignalConversion: '<S5>/TmpSignal ConversionAtGainInport1' incorporates:
   *  Gain: '<S5>/Gain'
   */
  tmp_1[0] = P4p2_B.Deg_2_rad[2];
  tmp_1[1] = P4p2_B.Deg_2_rad[3];
  tmp_1[2] = P4p2_B.Deg_2_rad[4];
  tmp_1[3] = P4p2_B.Deg_2_rad[5];
  tmp_1[4] = P4p2_B.Deg_2_rad[0];
  tmp_1[5] = P4p2_B.Deg_2_rad[1];
  ...
  for (i = 0; i < 3; i++) {
    tmp_2[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      tmp_2[i] += P4p2_P.C[3 * i_0 + i] * tmp_1[i_0];
    }
  }
  ...
  for (i = 0; i < 3; i++) {
    tmp_3[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      tmp_3[i] += P4p2_P.C[3 * i_0 + i] * P4p2_B.Integrator[i_0];
    }
  }
  ...
  for (i = 0; i < 6; i++) {
     ...
     *  Sum: '<S5>/Sum1'
     */
    tmp_1[i] = P4p2_P.L[i + 12] * unnamed_idx_2 + (P4p2_P.L[i + 6] *
      unnamed_idx_1 + P4p2_P.L[i] * u0);

    /* Gain: '<S5>/Gain1' incorporates:
     *  SignalConversion: '<S5>/TmpSignal ConversionAtGain1Inport1'
     *  Sum: '<S5>/Sum1'
     */
    tmp_4[i] = P4p2_P.B[i + 6] * rtb_TmpSignalConversionAtGain1I[1] + P4p2_P.B[i]
      * rtb_Backgain;
  }
  ...
  for (i = 0; i < 6; i++) {
    ...
    P4p2_B.Sum1_d[i] = (tmp_1[i] + tmp_4[i]) + u0;
  }
}
...
RT_MODEL_P4p2_T *P4p2(void)
{
  ...
  /* External mode info */
  P4p2_M->Sizes.checksums[0] = (1290896955U);
  P4p2_M->Sizes.checksums[1] = (3214001220U);
  P4p2_M->Sizes.checksums[2] = (2651204818U);
  P4p2_M->Sizes.checksums[3] = (927180567U);
  ...
  P4p2_M->Sizes.numBlocks = (63);      /* Number of blocks */
  P4p2_M->Sizes.numBlockIO = (25);     /* Number of block outputs */
