...
static void rate_monotonic_scheduler(void)
{
  ...
  P4p2_M->Timing.RateInteraction.TID0_2 = (P4p2_M->Timing.TaskCounters.TID[2] ==
    0);
...
void P4p2_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P4p2_M)) {
     ...
     */
    P4p2_B.Sum1 = P4p2_B.Gain_e + P4p2_P.offset_e_Value;
  }
  ...
  /* Gain: '<Root>/Deg_2_rad' */
  P4p2_B.Deg_2_rad[0] = P4p2_P.Deg_2_rad_Gain * P4p2_B.Gain;
  P4p2_B.Deg_2_rad[1] = P4p2_P.Deg_2_rad_Gain * P4p2_B.Gain_d;
  P4p2_B.Deg_2_rad[2] = P4p2_P.Deg_2_rad_Gain * P4p2_B.Gain_i;
  P4p2_B.Deg_2_rad[3] = P4p2_P.Deg_2_rad_Gain * P4p2_B.Gain_b;
  P4p2_B.Deg_2_rad[4] = P4p2_P.Deg_2_rad_Gain * P4p2_B.Sum1;
  P4p2_B.Deg_2_rad[5] = P4p2_P.Deg_2_rad_Gain * P4p2_B.Gain_dg;
  if (rtmIsMajorTimeStep(P4p2_M)) {
  }
  ...
  /* Sum: '<Root>/Sum2' */
  P4p2_B.Sum2[0] = P4p2_B.Integrator[4] - P4p2_B.Deg_2_rad[0];
  P4p2_B.Sum2[1] = P4p2_B.Integrator[5] - P4p2_B.Deg_2_rad[1];
  P4p2_B.Sum2[2] = P4p2_B.Integrator[0] - P4p2_B.Deg_2_rad[2];
  P4p2_B.Sum2[3] = P4p2_B.Integrator[1] - P4p2_B.Deg_2_rad[3];
  P4p2_B.Sum2[4] = P4p2_B.Integrator[2] - P4p2_B.Deg_2_rad[4];
  P4p2_B.Sum2[5] = P4p2_B.Integrator[3] - P4p2_B.Deg_2_rad[5];
  if (rtmIsMajorTimeStep(P4p2_M)) {
  }

  /* RateTransition: '<Root>/Rate Transition2' */
  if (rtmIsMajorTimeStep(P4p2_M) && P4p2_M->Timing.RateInteraction.TID0_2) {
    P4p2_B.RateTransition2[0] = P4p2_B.Gain;
    P4p2_B.RateTransition2[1] = P4p2_B.Gain_d;
    P4p2_B.RateTransition2[2] = P4p2_B.Gain_i;
    P4p2_B.RateTransition2[3] = P4p2_B.Gain_b;
    P4p2_B.RateTransition2[4] = P4p2_B.Sum1;
    P4p2_B.RateTransition2[5] = P4p2_B.Gain_dg;
  }
  ...
  rtb_Gain1[2] = P4p2_P.R2D1_Gain * P4p2_B.Integrator[0];
  rtb_Gain1[3] = P4p2_P.R2D1_Gain * P4p2_B.Integrator[1];
  rtb_Gain1[4] = P4p2_P.R2D1_Gain * P4p2_B.Integrator[2];
  rtb_Gain1[5] = P4p2_P.R2D1_Gain * P4p2_B.Integrator[3];

  /* RateTransition: '<Root>/Rate Transition1' */
  if (rtmIsMajorTimeStep(P4p2_M) && P4p2_M->Timing.RateInteraction.TID0_2) {
    for (i = 0; i < 6; i++) {
      P4p2_B.RateTransition1[i] = rtb_Gain1[i];
    }
  }
  ...
  rtb_Gain1[2] = P4p2_B.Deg_2_rad[4];
  rtb_Gain1[3] = P4p2_B.Deg_2_rad[5];
  rtb_Gain1[4] = P4p2_B.Deg_2_rad[0];
  rtb_Gain1[5] = P4p2_B.Deg_2_rad[1];

  /* Gain: '<S5>/Gain' */
  for (i = 0; i < 3; i++) {
    rtb_Sum[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      rtb_Sum[i] += P4p2_P.C[3 * i_0 + i] * rtb_Gain1[i_0];
    }
  }
  ...
  for (i = 0; i < 3; i++) {
    u0 = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      u0 += P4p2_P.C[3 * i_0 + i] * P4p2_B.Integrator[i_0];
    }
...
void P4p2_output2(void)                /* Sample time: [0.004s, 0.0s] */
{
  /* ToFile: '<Root>/To File' */
  {
    if (!(++P4p2_DW.ToFile_IWORK.Decimation % 1) && (P4p2_DW.ToFile_IWORK.Count*
         7)+1 < 100000000 ) {
      ...
      if (fp != (NULL)) {
        ...
        u[1] = P4p2_B.RateTransition2[0];
        u[2] = P4p2_B.RateTransition2[1];
        u[3] = P4p2_B.RateTransition2[2];
        u[4] = P4p2_B.RateTransition2[3];
        u[5] = P4p2_B.RateTransition2[4];
  ...
  /* ToFile: '<Root>/To File2' */
  {
    if (!(++P4p2_DW.ToFile2_IWORK.Decimation % 1) &&
        (P4p2_DW.ToFile2_IWORK.Count*7)+1 < 100000000 ) {
      FILE *fp = (FILE *) P4p2_DW.ToFile2_PWORK.FilePtr;
      if (fp != (NULL)) {
        real_T u[7];
        P4p2_DW.ToFile2_IWORK.Decimation = 0;
        u[0] = P4p2_M->Timing.t[2];
        u[1] = P4p2_B.RateTransition1[0];
        u[2] = P4p2_B.RateTransition1[1];
        u[3] = P4p2_B.RateTransition1[2];
        u[4] = P4p2_B.RateTransition1[3];
        u[5] = P4p2_B.RateTransition1[4];
        ...
        if (((++P4p2_DW.ToFile2_IWORK.Count)*7)+1 >= 100000000) {
          (void)fprintf(stdout,
                        "*** The ToFile block will stop logging data before\n"
                        "    the simulation has ended, because it has reached\n"
                        "    the maximum number of elements (100000000)\n"
...
RT_MODEL_P4p2_T *P4p2(void)
{
...

  {
    ...
    for (i = 0; i < 6; i++) {
      P4p2_B.RateTransition2[i] = 0.0;
    }

    for (i = 0; i < 6; i++) {
      P4p2_B.RateTransition1[i] = 0.0;
    }
