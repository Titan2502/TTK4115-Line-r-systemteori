 ...
 * C source code generated on : Thu Oct 18 18:28:27 2018
 *
...
void P2p2_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P2p2_M)) {
    ...
    /* Gain: '<Root>/R2D' */
    P2p2_B.R2D = P2p2_P.R2D_Gain * P2p2_B.Joystick_gain_x;
  ...
  if (rtmIsMajorTimeStep(P2p2_M)) {
     ...
     */
    P2p2_B.Sum1 = P2p2_B.Gain_e + P2p2_P.offset_e_Value;
  }
  ...
  /* Gain: '<Root>/Deg_2_rad' */
  P2p2_B.Deg_2_rad[0] = P2p2_P.Deg_2_rad_Gain * P2p2_B.Gain_p;
  P2p2_B.Deg_2_rad[1] = P2p2_P.Deg_2_rad_Gain * P2p2_B.Gain;
  P2p2_B.Deg_2_rad[2] = P2p2_P.Deg_2_rad_Gain * P2p2_B.Gain_i;
  P2p2_B.Deg_2_rad[3] = P2p2_P.Deg_2_rad_Gain * P2p2_B.Gain_b;
  P2p2_B.Deg_2_rad[4] = P2p2_P.Deg_2_rad_Gain * P2p2_B.Sum1;
  P2p2_B.Deg_2_rad[5] = P2p2_P.Deg_2_rad_Gain * P2p2_B.Gain_d;

  /* Gain: '<S6>/K_rp' incorporates:
   *  Sum: '<S6>/Sum'
   */
  rtb_Frontgain = (P2p2_B.Joystick_gain_x - P2p2_B.Deg_2_rad[1]) * P2p2_P.K_rp;
    ...
    if (rtmIsMajorTimeStep(P2p2_M)) {
      {
        if (!(++P2p2_DW.ToFile_IWORK.Decimation % 1) &&
          ...
          if (fp != (NULL)) {
            ...
            u[1] = P2p2_B.Joystick_gain_x;
            if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
              rtmSetErrorStatus(P2p2_M,
                                "Error writing to MAT-file p2p2_travelRate_reference.mat");
              return;
            }
            ...
            if (((++P2p2_DW.ToFile_IWORK.Count)*2)+1 >= 100000000) {
              (void)fprintf(stdout,
                            ...
                            "    allowed in MAT-file p2p2_travelRate_reference.mat.\n");
            }
          }
        }
      }
    ...
    if (rtmIsMajorTimeStep(P2p2_M)) {
      {
        if (!(++P2p2_DW.ToFile1_IWORK.Decimation % 1) &&
            (P2p2_DW.ToFile1_IWORK.Count*7)+1 < 100000000 ) {
          FILE *fp = (FILE *) P2p2_DW.ToFile1_PWORK.FilePtr;
          if (fp != (NULL)) {
            real_T u[7];
            P2p2_DW.ToFile1_IWORK.Decimation = 0;
            u[0] = P2p2_M->Timing.t[1];
            u[1] = P2p2_B.Deg_2_rad[0];
            u[2] = P2p2_B.Deg_2_rad[1];
            u[3] = P2p2_B.Deg_2_rad[2];
            u[4] = P2p2_B.Deg_2_rad[3];
            u[5] = P2p2_B.Deg_2_rad[4];
            u[6] = P2p2_B.Deg_2_rad[5];
            if (fwrite(u, sizeof(real_T), 7, fp) != 7) {
              rtmSetErrorStatus(P2p2_M,
                                "Error writing to MAT-file p2p2_state_output.mat");
              return;
            }

            if (((++P2p2_DW.ToFile1_IWORK.Count)*7)+1 >= 100000000) {
              (void)fprintf(stdout,
                            "*** The ToFile block will stop logging data before\n"
                            "    the simulation has ended, because it has reached\n"
                            "    the maximum number of elements (100000000)\n"
                            "    allowed in MAT-file p2p2_state_output.mat.\n");
            }
          }
        }
      }
  ...
  if (rtmIsMajorTimeStep(P2p2_M)) {
    /* Constant: '<Root>/e_ref' */
    P2p2_B.e_ref = P2p2_P.e_ref_Value;
  }
  ...
  rtb_Backgain = ((P2p2_P.K_ep_Gain * rtb_Sum_p + rtb_Backgain) -
                  P2p2_P.K_ed_Gain * P2p2_B.Deg_2_rad[5]) + P2p2_B.Vs_const;
...
void P2p2_initialize(void)
{
  ...
  {
    char fileName[509] = "p2p2_travelRate_reference.mat";
    FILE *fp = (NULL);
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(P2p2_M,
                        "Error creating .mat file p2p2_travelRate_reference.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,2,0,"travelRate_ref")) {
      rtmSetErrorStatus(P2p2_M,
                        "Error writing mat file header to file p2p2_travelRate_reference.mat");
      return;
    }
  ...
  {
    char fileName[509] = "p2p2_state_output.mat";
    FILE *fp = (NULL);
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(P2p2_M, "Error creating .mat file p2p2_state_output.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,7,0,"states")) {
      rtmSetErrorStatus(P2p2_M,
                        "Error writing mat file header to file p2p2_state_output.mat");
      return;
    }
...
void P2p2_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      ...
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(P2p2_M,
                          "Error closing MAT-file p2p2_travelRate_reference.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(P2p2_M,
                          "Error reopening MAT-file p2p2_travelRate_reference.mat");
        return;
      }
      ...
      if (rt_WriteMat4FileHeader(fp, 2, P2p2_DW.ToFile_IWORK.Count,
           "travelRate_ref")) {
        rtmSetErrorStatus(P2p2_M,
                          "Error writing header for travelRate_ref to MAT-file p2p2_travelRate_reference.mat");
      }
  ...
  /* Terminate for ToFile: '<Root>/To File1' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "p2p2_state_output.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(P2p2_M, "Error closing MAT-file p2p2_state_output.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(P2p2_M,
                          "Error reopening MAT-file p2p2_state_output.mat");
        return;
      }
      ...
      {
        rtmSetErrorStatus(P2p2_M,
                          "Error writing header for states to MAT-file p2p2_state_output.mat");
      }
...
RT_MODEL_P2p2_T *P2p2(void)
{
  ...
  /* External mode info */
  P2p2_M->Sizes.checksums[0] = (2477459782U);
  P2p2_M->Sizes.checksums[1] = (108180846U);
  P2p2_M->Sizes.checksums[2] = (2432483396U);
  P2p2_M->Sizes.checksums[3] = (3090274882U);
  ...
  P2p2_M->Sizes.numBlocks = (62);      /* Number of blocks */
