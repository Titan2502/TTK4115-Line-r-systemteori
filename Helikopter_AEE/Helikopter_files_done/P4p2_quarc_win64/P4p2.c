 ...
 * C source code generated on : Thu Oct 18 20:31:11 2018
 *
...
void P4p2_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(P4p2_M)) {
     ...
     */
    P4p2_B.P[0] = 0.0;
    P4p2_B.P[0] += P4p2_P.P[0] * P4p2_B.Joystick_gain_x;
    P4p2_B.P[0] += P4p2_P.P[2] * P4p2_B.Joystick_gain_y;
    P4p2_B.P[1] = 0.0;
    P4p2_B.P[1] += P4p2_P.P[1] * P4p2_B.Joystick_gain_x;
    P4p2_B.P[1] += P4p2_P.P[3] * P4p2_B.Joystick_gain_y;
  }
   ...
   */
  for (i = 0; i < 2; i++) {
    rtb_Sum[i] = P4p2_B.P[i] - ((P4p2_P.K[i + 2] * P4p2_B.Integrator[1] +
      P4p2_P.K[i] * P4p2_B.Integrator[0]) + P4p2_P.K[i + 4] * P4p2_B.Integrator
      [3]);
  }
   ...
   *  Sum: '<S1>/Add'
   */
  u0 = (rtb_Sum[1] + rtb_Sum[0]) * P4p2_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  u0 = (rtb_Sum[0] - rtb_Sum[1]) * P4p2_P.Backgain_Gain;
   ...
   *  Gain: '<S5>/Gain'
   */
  tmp[0] = P4p2_B.Deg_2_rad[2];
  tmp[1] = P4p2_B.Deg_2_rad[3];
  tmp[2] = P4p2_B.Deg_2_rad[4];
  tmp[3] = P4p2_B.Deg_2_rad[5];
  tmp[4] = P4p2_B.Deg_2_rad[0];
  tmp[5] = P4p2_B.Deg_2_rad[1];
  ...
  for (i = 0; i < 3; i++) {
    tmp_0[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      tmp_0[i] += P4p2_P.C[3 * i_0 + i] * tmp[i_0];
    }
  }
  ...
  for (i = 0; i < 3; i++) {
    tmp_1[i] = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      tmp_1[i] += P4p2_P.C[3 * i_0 + i] * P4p2_B.Integrator[i_0];
    }
  }
  ...
  for (i = 0; i < 6; i++) {
     ...
     *  Sum: '<S5>/Sum1'
     */
    tmp[i] = P4p2_P.L[i + 12] * unnamed_idx_2 + (P4p2_P.L[i + 6] * unnamed_idx_1
      + P4p2_P.L[i] * u0);

    /* Gain: '<S5>/Gain1' incorporates:
     *  Sum: '<S5>/Sum1'
     */
    tmp_2[i] = P4p2_P.B[i + 6] * rtb_Sum[1] + P4p2_P.B[i] * rtb_Sum[0];
  }
  ...
  for (i = 0; i < 6; i++) {
    u0 = 0.0;
    for (i_0 = 0; i_0 < 6; i_0++) {
      u0 += P4p2_P.A[6 * i_0 + i] * P4p2_B.Integrator[i_0];
    }

    P4p2_B.Sum1_f[i] = (tmp[i] + tmp_2[i]) + u0;
  }
}
...
RT_MODEL_P4p2_T *P4p2(void)
{
  ...
  /* External mode info */
  P4p2_M->Sizes.checksums[0] = (1387726556U);
  P4p2_M->Sizes.checksums[1] = (388849031U);
  P4p2_M->Sizes.checksums[2] = (3264952187U);
  P4p2_M->Sizes.checksums[3] = (2925702284U);
  ...
  P4p2_M->Sizes.numBlocks = (59);      /* Number of blocks */
  P4p2_M->Sizes.numBlockIO = (23);     /* Number of block outputs */
  P4p2_M->Sizes.numBlockPrms = (238);  /* Sum of parameter "widths" */
  return P4p2_M;
}
